...
/**
 * S-07 緊急度結果画面（患者向け）。
 * final_level の定型文言・科目候補・免責のみを表示する。
...
export default async function ResultPage({
  ...
  if (q.status === "ai_interview") redirect(`/questionnaire/${id}/interview`);

  const triage = await triageRepo().latest(ctx, id);
  const level: TriageLevel = triage?.finalLevel ?? "L3";
  ...
  return (
    <main className="grid gap-4">
      ...
      {q.suggestedDepartments.length > 0 && (
        <div className="rounded-2xl border border-line bg-surface p-5">
          <p className="text-[15px] font-bold">ご相談先の候補</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {q.suggestedDepartments.map((d) => (
              <span key={d}
